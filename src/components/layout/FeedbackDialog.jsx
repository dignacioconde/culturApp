  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Cuéntanos qué mejorar">
      <form className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
        ...
        <label className="flex items-start gap-3 rounded-lg border border-border-subtle bg-surface-muted p-4 text-sm text-text-primary">
          <input
            id={consentId}
            type="checkbox"
            ...
            className="mt-1 h-5 w-5 shrink-0 rounded border-border-subtle accent-accent-primary"
          />
          <span className="min-w-0">
            <span className="block font-medium">Acepto enviar este comentario para mejorar la beta.</span>
            <span className="mt-1 block text-text-secondary">No incluyas datos sensibles de clientes.</span>
            {consentError && <span id={consentErrorId} className="mt-2 block text-xs font-medium text-danger">{consentError}</span>}
          </span>
        </label>
