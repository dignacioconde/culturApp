...
- **Flujo para implementacion**: Para tareas pequeñas (fixes, chores, mejoras menores), leer solo `AGENTS.md`, `.memory/MEMORY.md`, `docs/project/START_HERE.md` y la issue `CACH-*` relacionada si existe. Crear rama desde `main` y abrir PR a `main`. Leer `CURRENT_RELEASE.md`, `CURRENT_PLAN.md` y `BACKLOG.md` solo si la tarea pertenece a una release activa, afecta planificación o cambia alcance de producto. No se exige release activa para fixes, chores ni features menores. Ver `docs/project/process/WORKFLOW.md`.
